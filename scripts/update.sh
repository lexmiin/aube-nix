...
Options:
  --version VERSION  Update to a specific aube version, with or without leading v.
...
if [[ ! -f flake.nix || ! -f "$PACKAGE_FILE" ]]; then
  printf 'Run this script from the aube-nix repository root.\n' >&2
  exit 1
fi
...
for pair in "${targets[@]}"; do
  system="${pair%%:*}"
  target="${pair#*:}"
  asset_name="aube-v${target_version}-${target}.tar.gz"
...
if [[ "$skip_build" == false ]]; then
  ...
  nix build .#aube --print-build-logs
  ./result/bin/aube --version
fi
